...
const BookCard: React.FC<BookCardProps> = ({ book, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
      ...
      {/* Book Information */}
      <div className="p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
        <Link href={`/book/${book.id}`} className="block cursor-pointer">
          <h3 className="text-lg font-semibold text-gray-800 truncate hover:text-blue-600 hover:underline transition-colors duration-200">{book.title}</h3>
        ...
        <div className="flex gap-2 mt-4">
          <Link href={`/book/${book.id}`} className="flex-1 cursor-pointer">
            <button className="w-full px-3 py-2 text-sm bg-green-600 border border-green-700 text-gray-700 rounded-md hover:bg-green-700 transition-colors duration-200 cursor-pointer">
              View Details
            </button>
          </Link>
